...
export default function ResourceClientPage({ resources, lastUpdated }: ResourceClientPageProps) {
  const [searchQuery, setSearchQuery] = useState("");
  const [selectedTag, setSelectedTag] = useState("all");
  const [visibleItems, setVisibleItems] = useState(6);

  // Revalidate için konsol mesajı
  useEffect(() => {
    if (lastUpdated) {
      console.log(`%c 🔄 CLIENT COMPONENT YENİDEN RENDER EDİLDİ - Kaynak Sayısı: ${resources.length} `, 'background: #059669; color: white; padding: 4px; border-radius: 4px; font-weight: bold;');
      console.log(`%c Son Güncelleme: ${lastUpdated} `, 'background: #fde047; color: black; padding: 2px; border-radius: 4px;');
    }
